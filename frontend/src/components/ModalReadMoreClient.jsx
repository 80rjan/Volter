...
export default function ModalReadMoreClient({client, updateTelephones, closeModal}) {
    ...
    const [telephones, setTelephones] = React.useState({
        telephone1: client["Telephone 1"], telephone2: client["Telephone 2"],
    });
    ...
    const updateTelephoneNumbers = () => {
        ...
        axios.post("http://localhost:3000/clients/updateTelephones", {
            clientId: client.Id, telephone1: telephones.telephone1, telephone2: telephones.telephone2,
        })
            .then((res) => {
                console.log("Telephones updated successfully");
                updateTelephones(telephones.telephone1, telephones.telephone2);
                setUpdateTelephone(false);
            })
            .catch((err) => {
                console.error("Error updating telephones:", err);
            })
        ...
        <Wrapper>
            ...
            <ClientInfo>
                <div>
                    <UserRound size={32}/>
                    {client.Name}
                    <div>({client["Date Joined"].split("T")[0]})</div>
                </div>
                <div>
                    <span>
                      <p>Град:</p>
                    ...
                    <span>
                        ...
                        {loadingTelephoneUpdate ? (<Loading width={32} height={32}/>) : updateTelephone ? (
                            <div style={{display: "flex", alignItems: "center", gap: "0.4rem"}}>
                                <input type="text" defaultValue={client["Telephone 1"]}
                                       onChange={e => setTelephones({
                                           ...telephones, telephone1: e.target.value
                                       })}/>
                                <input type="text" defaultValue={client["Telephone 2"]}
                                       onChange={e => setTelephones({
                                           ...telephones, telephone2: e.target.value
                                       })}/>
                                <button onClick={updateTelephoneNumbers} style={{}} type={"button"}>
                                    Зачувај
                                </button>
                                <X onClick={() => setUpdateTelephone(false)}/>
                            </div>
                        ) : (
                            <>
                                <p>
                                ...
                                <button onClick={() => setUpdateTelephone(true)} type={"button"}>
                                    Измени/Додади телефон
                                </button>
                            </>)}
                    </span>
                </div>
            ...
            <Tables>
                <div>
                    ...
                    <Table>
                        ...
                        <ScrollableData>
                            {loading ? Array.from({length: 50}).map((_, index) => (<SkeletonRow
                                key={index}
                                style={index % 2 === 1 ? {background: "#f0f0f0"} : {background: "#ffffff"}}
                            />)) : pawns.map((pawn, index) => (<DataShort
                                key={index}
                                style={index % 2 === 1 ? {background: "#f0f0f0"} : {background: "#ffffff"}}
                            >
                          ...
                          {Number(pawn.provision).toLocaleString("de-DE")}
                                ...
                                <span
                                    className={`bold ${pawn.days_left < 0 ? "red" : "green"}`}
                                >
                          {pawn.days_left}
                        </span>
                    ...
                    <Table>
                        ...
                        <ScrollableData>
                            {loading ? Array.from({length: 50}).map((_, index) => (<SkeletonRow
                                key={index}
                                style={index % 2 === 1 ? {background: "#f0f0f0"} : {background: "#ffffff"}}
                            />)) : transactions.map((transaction, index) => (<DataShort
                                key={index}
                                style={index % 2 === 1 ? {background: "#f0f0f0"} : {background: "#ffffff"}}
                            >
                                ...
                                <span>
                          {Number(transaction.money_given).toLocaleString("de-DE")}
                        </span>
                                <span>
                          {Number(transaction.money_got).toLocaleString("de-DE")}
                        </span>
                                <span>
                          {Number(transaction.profit).toLocaleString("de-DE")}
                        </span>
                                <span
                                    className={`bold color ${Number(transaction.money_diff) === 0 ? "" : Number(transaction.money_diff) < 0 ? "red" : "green"}`}
                                >
                          {Number(transaction.money_diff).toLocaleString("de-DE")}
                        </span>
                                <span style={{whiteSpace: "nowrap"}}>
