...
export default function ModalAddNewPawn({ closeModal, refresh }) {
  ...
  const fetchClients = (limit, offset, search) => {
    axios
      .get(
        `http://localhost:3000/clientsAutocomplete?limit=${limit}&offset=${offset}&search=${search}`
      )
      .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    axios
      .post(`http://localhost:3000/insertPawn`, formData)
      .then((response) => {
        closeModal();
        refresh();
      })
    ...
    <>
      ...
      <Wrapper>
        ...
        <Form onSubmit={handleSubmit}>
          <div>
            <ClientInputs>
              <span style={{ width: "max-content", marginBottom: ".8rem" }}>
              ...
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="Постоечки клиенти" />
                )}
      ...
      <div>
        ...
        <StyledInput
          ...
          onChange={(e) => {
            ...
            handleInputChange(
              "provisionPercent",
              Math.round((formData.provision / e.target.value) * 100 * 100) /
                100
            );
          }}
          type="number"
            ...
            <StyledInput
              ...
              onChange={(e) => {
                ...
                handleInputChange(
                  "provisionPercent",
                  Math.round(
                    (e.target.value / formData.price_pawned) * 100 * 100
                  ) / 100
                );
              }}
              type="number"
          ...
          <span>
            <StyledInput
              ...
              onChange={(e) => {
                ...
                handleInputChange(
                  "provision",
                  Math.round((e.target.value / 100) * formData.price_pawned)
                );
              }}
              type="number"
              step="0.001"
