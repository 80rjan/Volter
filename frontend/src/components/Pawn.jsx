...
export default function Pawn({ pawn, refresh, isOdd, refreshCashReg }) {
  ...
  const continuePawn = (
    ...
    axios
      .put(`http://localhost:3000/continuePawn`, {
        id,
        tableName,
        provision,
        description,
  ...
  const closePawn = (id, category, priceClosed, description) => {
    ...
    axios
      .put(`http://localhost:3000/closePawn`, {
        id,
        tableName,
        priceClosed,
        description,
      ...
      .then(() => {
        ...
        setInfoMsg(
          `Додадени се ${priceClosed.toLocaleString("de-DE")} во каса!`
        );
  ...
  const movePawnToSale = (id, category) => {
    ...
    axios
      .put(`http://localhost:3000/changePawnToSale`, { id, tableName })
      .then(() => {
  ...
  const fetchPawn = (clientId, category, pawnId) => {
    ...
    axios
      .get(
        `http://localhost:3000/getPawn?clientId=${clientId}&category=${category}&pawnId=${pawnId}`
      )
      .then((res) => {
  ...
  return (
    <Wrapper
      style={isOdd ? { background: "#f0f0f0" } : { background: "#ffffff" }}
    >
      <Text>{pawn["Client Id"]}</Text>
      <Text className={`bold ${pawn["Days Left"] < 0 && "red"}`}>
        {pawn.Name.toUpperCase()}
      </Text>
      ...
      <Text className={`bold ${pawn["Days Left"] < 0 ? "red" : "green"}`}>
        {pawn["Days Left"]}
      </Text>
        ...
        <>
          <ButtonWrapper>
            ...
            <RotateCcw
              size={16}
              color="var(--cta-color)"
              onClick={() => setModalContinuePawn(true)}
            />
            <Euro
              size={16}
              color="var(--green)"
              onClick={() => movePawnToSale(pawn.Id, pawn.Category)}
            />
          </ButtonWrapper>
          <Ellipsis
            size={20}
      ...
      {modalClosePawn && (
        <ModalActions
          ...
          dailyProvision={Math.abs(
            Math.round(pawn.Provision) / Number(pawn["Total Days"])
          )}
...
const Wrapper = styled.div`
  display: grid;
  place-items: center;
  grid-template-columns: 3rem 1.5fr 1fr 2fr repeat(4, 1fr) 1.5fr 0.5fr;
  //padding: .2rem;
