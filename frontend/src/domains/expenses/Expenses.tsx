...
import { ExpenseMonthlySummary, ExpenseCategory, EXPENSE_CATEGORIES, EXPENSE_CATEGORY_LABEL, MONTHS_MK } from "./types.ts";
...
export default function Expenses() {
    ...
    return (
        <div className="h-screen flex pl-16">
                    ...
                    <>
                        <div className="flex justify-between items-center w-full gap-3 flex-wrap">
                            <div className="flex gap-3 items-center flex-wrap">
                                ...
                                {(filterStaff || filterCategory || filterFrom || filterTo) && (
                                    <button onClick={() => { setFilterStaff(""); setFilterCategory(""); setFilterFrom(""); setFilterTo(""); }}
                        ...
                        <div className="flex flex-col bg-white rounded-lg shadow-[0_0_8px_rgba(0,0,0,0.2)] overflow-hidden flex-1 min-h-0">
                            <div className={`grid place-items-center ${cols} gap-2 px-2 py-2 border-b-2 border-black/20 text-[#eee] bg-[#666] text-xs font-medium`}>
                                <div className="flex items-center cursor-pointer" onClick={() => handleOrder("Month")}>Месец <SortIcon dir={dirOf("Month")} /></div>
                                {EXPENSE_CATEGORIES.map(c => (
                                    <div key={c} className="flex items-center cursor-pointer" onClick={() => handleOrder(c)}>{EXPENSE_CATEGORY_LABEL[c]} <SortIcon dir={dirOf(c)} /></div>
                                ))}
                                <div className="flex items-center cursor-pointer" onClick={() => handleOrder("Total")}>Вкупно <SortIcon dir={dirOf("Total")} /></div>
                                <div></div>
                            </div>
                            ...
                            <div className="overflow-y-auto overflow-x-hidden flex-1 scrollbar-thin svg-hover">
                                {loading ? <Loading /> : sorted.length === 0 ? (
                                    ...
                                    >
                                        ...
                                        <Ellipsis size={18} color="#888" className="cursor-pointer" onClick={() => setSelected(s)} />
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            {modalAdd && <ModalAddNewExpense closeModal={() => setModalAdd(false)} refresh={() => setRefresh(p => !p)} />}
            {selected && <ModalReadMoreExpenseMonth summary={selected} staffId={filterStaff || null} closeModal={() => setSelected(null)} />}
        </div>
    );
}
