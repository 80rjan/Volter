                    ...
                    <>
                        <div className="flex justify-between items-center w-full gap-3 flex-wrap">
                            <div className="flex gap-3 items-center flex-wrap">
                                ...
                                <input className={inputClass} type="search" placeholder="Пребарувај по корисничко" value={searchUsername} onChange={e => setSearchUsername(e.target.value)} />
                                <select className={inputClass} style={{ color: filterStatus === "" ? "#888" : "#000" }} value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
                                    <option value="">Сите статуси</option>
                                    {(Object.keys(STAFF_STATUS_LABEL) as StaffStatus[]).map(s => <option key={s} value={s}>{STAFF_STATUS_LABEL[s]}</option>)}
                                </select>
                            ...
                            <button
                                className="flex items-center gap-2 bg-green h-fit text-white rounded px-5 py-2 text-sm shadow-[4px_2px_6px_rgba(0,0,0,0.2)] transition-all duration-300 hover:scale-105 group shrink-0"
                                onClick={() => setModalAdd(true)}
                            >
                                <Plus size={18} color="white" strokeWidth={3} className="transition-transform duration-500 group-hover:rotate-90" />
                                Внеси нов вработен
                            </button>
                        </div>
                        ...
                        <div className="flex flex-col bg-white rounded-lg shadow-[0_0_8px_rgba(0,0,0,0.2)] overflow-hidden flex-1 min-h-0">
                            <div className={`grid place-items-center ${cols} gap-2 px-2 py-2 border-b-2 border-black/20 text-[#eee] bg-[#666] text-xs font-medium`}>
                                <div className="flex items-center cursor-pointer" onClick={() => handleOrder("Id")}>Код <SortIcon dir={dirOf("Id")} /></div>
                                <div className="flex items-center cursor-pointer" onClick={() => handleOrder("Name")}>Име <SortIcon dir={dirOf("Name")} /></div>
                                <div className="flex items-center cursor-pointer" onClick={() => handleOrder("Username")}>Корисничко име<SortIcon dir={dirOf("Username")} /></div>
                                <div className="flex items-center cursor-pointer" onClick={() => handleOrder("Status")}>Статус <SortIcon dir={dirOf("Status")} /></div>
                                <div>Менаџер</div>
                                <div className="flex items-center cursor-pointer" onClick={() => handleOrder("Date")}>Креиран <SortIcon dir={dirOf("Date")} /></div>
                                <div></div>
                            </div>

                            <div className="overflow-y-auto overflow-x-hidden flex-1 scrollbar-thin svg-hover">
                                {loading ? <Loading /> : sorted.length === 0 ? (
                                    <p className="text-center text-sm text-[#888] py-6">Нема вработени за прикажување.</p>
                                ) : sorted.map((s, index) => (
                                    <div
                                        key={s.id}
                                    ...
                                    >
                                        ...
                                        <Ellipsis size={18} color="#888" className="cursor-pointer" onClick={() => setSelectedId(s.id)} />
                                    </div>
                                ))}
                            </div>
                        </div>
