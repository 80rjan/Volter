...
export default function Pawns() {
  ...
  const fetchPawns = (
    ...
    axios
      .get(
        `http://localhost:3000?limit=${limit}&offset=${offset}&orderBy=${order}&orderDirection=${direction}&searchByName=${searchByName}&searchByEmbg=${searchByEmbg}&searchByTel=${searchByTel}&searchByCategory=${searchByCategory}`
      )
      .then((res) => {
        setSummary(res.data.summary);

        const pawns = res.data.pawns;
        const newUnique = pawns.filter(
          (p) => !fetchedPawnIds.current.has(`${p.Category}_${p.Id}`)
        );
        newUnique.forEach((p) =>
          fetchedPawnIds.current.add(`${p.Category}_${p.Id}`)
        );
    ...
    <PawnsPage>
      ...
      <Container>
        <FilterWrapper>
          ...
          <ButtonAddNewPawn onClick={() => setModalAddNewPawn(true)}>
        ...
        <PawnsWrapper>
          ...
          {(searchByTel.length > 0 ||
            searchByEmbg.length > 0 ||
              ...
              <div>
                ...
                <span>
                  {loading ? (
                    ...
                    ((summary["Provision"] / summary["Money Pawns"]) * 100 || 0)
                      .toFixed(2)
        ...
        <CashRegister
          ...
          refreshDependencyAdjustPawn={refreshCashReg}
        />
      </Container>
    </PawnsPage>
  );
