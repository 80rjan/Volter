export function numberInWordsMkd(number: number): string {
    ...
    function threeDigitsToWords(n: number): string {
        ...
        if (threeDigits > 0) result += stotki[threeDigits] + ' ';

        if (twoDigits > 9 && twoDigits < 20) {
            result += teen[twoDigits - 10] + ' ';
        } else {
            if (Math.floor(twoDigits / 10) > 1) result += desetki[Math.floor(twoDigits / 10)] + ' ';
            if (oneDigit > 0 && Math.floor(twoDigits / 10) !== 1) result += edinici[oneDigit] + ' ';
        }
    ...
    if (millions > 0) words += millions === 1 ? 'еден милион ' : threeDigitsToWords(millions) + ' милиони ';
    if (thousands > 0) words += thousands === 1 ? 'илјада ' : threeDigitsToWords(thousands) + ' илјади ';
    if (other > 0) words += threeDigitsToWords(other);
