package com.volter.shared.specification;
...
public class PredicateBuilder<T> {
    ...
    /** Matches {@code value} (case-insensitive, partial) against any of the given paths (OR'd together). */
    @SafeVarargs
    public final PredicateBuilder<T> withAnyStringLike(String value, Path<String>... paths) {
        if (value != null && !value.isBlank() && paths.length > 0) {
            String pattern = "%" + value.toLowerCase() + "%";
            List<Predicate> ors = new ArrayList<>(paths.length);
